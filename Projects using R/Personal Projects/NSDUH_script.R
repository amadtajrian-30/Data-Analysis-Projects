...
# Age groups
age_weighted_by_gender <- svytable(~CATAG3 + gender, design = nsduh_merged_design, na.action = na.pass) # Weighted N
print(age_weighted_by_gender)
age_weighted_by_gender_pct <- svytable(~CATAG3 + gender, design = nsduh_merged_design, Ntotal = TRUE, na.action = na.pass)
print(round(age_weighted_by_gender_pct * 100, 2))
...
print(svychisq(~CATAG3 + gender, design = nsduh_merged_design, statistic = "Chisq", na.action = na.omit)) # Chi sqaure test

# Education level - combine 4 & 5
...
smi_unweighted_gender_table_counts <- table(nsduh_merged$smi_past_year, nsduh_merged$gender, useNA = "always")
...
mrj_unweighted_gender_table_counts <- table(nsduh_merged$mrj_disorder, nsduh_merged$gender, useNA = "always")
...
mentalhlth_adj_model <- svyglm(received_mh_tx ~ gender + CATAG3 + EDUHIGHCAT + INCOME + POVERTY3,
                               design = nsduh_merged_design,
                               family = quasibinomial(),
                               na.action = na.omit)
...
sudtrt_adj_model <- svyglm(received_sud_tx ~ gender + CATAG3 + EDUHIGHCAT + INCOME + POVERTY3,
                               design = nsduh_merged_design,
                               family = quasibinomial(),
                               na.action = na.omit)
